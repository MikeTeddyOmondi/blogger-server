...
router.post('/contacts', async(req, res) => {
    ...
    autoContactMail(mail_body)
        ...
        .catch((error) => console.log(`Error occurred while sending the automatic email to the subscriber: ${ error.message }`))
})
...
router.post('/subscribers', async(req, res) => {
    ...
    subscriber.save()
        ...
        .catch(err => {
            res.status(500).json({ error: `Error occurred while saving a subscriber: ${ err.message }` })
        })
